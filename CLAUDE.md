...
- **Configurações**: toggle de tema, "Gerenciar jogadores locais" (renomear/excluir — exclusão bloqueada se o jogador tem histórico de partidas), botões **"Importar JSON"** (`OutlinedButton` + `Icons.Filled.FileDownload`) e **"Exportar JSON"** (`OutlinedButton` + `Icons.Filled.FileUpload`, mesmo padrão visual) — ver seção "Importação/exportação de jogos via JSON" abaixo.

## Importação/exportação de jogos via JSON

Botões "Importar JSON" e "Exportar JSON" em Configurações — os dois lêem/escrevem o **mesmo formato** de arquivo (`SeedGamesFile`), documentado e exemplificado em `Regras/Jogos iniciais.json` (jogo "7 Wonders" + seu schema de pontuação, extraídos do Ludopedia + das regras oficiais). Exportar e reimportar o mesmo arquivo é um round-trip válido.
